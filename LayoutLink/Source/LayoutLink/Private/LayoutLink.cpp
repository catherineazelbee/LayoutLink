...
void FLayoutLinkModule::LoadSettings()
{
	...
	if (AssetLibraryPath.IsEmpty())
		AssetLibraryPath = DefaultAsset;
	if (LayoutExportDir.IsEmpty())
		LayoutExportDir = DefaultLayouts;
}
	...
	return SNew(SDockTab)
						...
						+ SHorizontalBox::Slot()
							  ...
							  .VAlign(VAlign_Center)
								  [SNew(SEditableTextBox)
									   .Text_Lambda([this]()
													{ return FText::FromString(AssetLibraryPath); })
					   ...
					   [SNew(SHorizontalBox)
						...
						+ SHorizontalBox::Slot()
							  ...
							  .VAlign(VAlign_Center)
								  [SNew(SEditableTextBox)
									   .Text_Lambda([this]()
													{ return FText::FromString(LayoutExportDir); })
									   .OnTextCommitted_Lambda([this](const FText &NewText, ETextCommit::Type)
															   {
		...
		if (StatusTextWidget.IsValid())
		{
			StatusTextWidget->SetText(FText::FromString(
			 ...
			 // Frame Range
			 + SScrollBox::Slot()
				   .Padding(10.f, 5.f, 10.f, 3.f)
					   [SNew(STextBlock)
							.Text(LOCTEXT("FrameRangeLabel", "Frame Range:"))]
				   ...
				   .Padding(10.f, 0.f, 10.f, 10.f)
					   [SNew(SHorizontalBox)
						...
						+ SHorizontalBox::Slot()
							  ...
							  .Padding(0.f, 0.f, 5.f, 0.f)
							  .VAlign(VAlign_Center)
								  [SNew(STextBlock).Text(LOCTEXT("End", "End:"))]
...
FReply FLayoutLinkModule::OnGetFrameRangeFromSequence()
{
	// Try to get the currently open Level Sequence and read its range
	...
	// Python script to get current sequence range
	const FString Command = FString::Printf(
		TEXT("import sys; sys.path.insert(0, r'%s'); ")
			TEXT("import unreal; ")
				TEXT("try:\n")
					TEXT("    seq = unreal.LevelSequenceEditorBlueprintLibrary.get_current_level_sequence()\n")
						TEXT("    if seq:\n")
							TEXT("        start = int(seq.get_playback_start())\n")
								TEXT("        end = int(seq.get_playback_end())\n")
									TEXT("        print(f'FRAME_RANGE:{start},{end}')\n")
										TEXT("    else:\n")
											TEXT("        print('No sequence open')\n")
	...
	// Note: For now, this just logs to console
	// To actually update the spinboxes, you'd need to parse Python output
	// For simplicity, users can just type the values manually

	if (StatusTextWidget.IsValid())
	{
		StatusTextWidget->SetText(FText::FromString(
			TEXT("Check Output Log for sequence frame range.\n\nManually update Start/End fields.")));
	}
			...
			TEXT("import layout_export as le; ")
				TEXT("print('=== Layout Export Starting ==='); ")
					TEXT("print('Layout file:', r'%s'); ")
						TEXT("print('Asset library:', r'%s'); ")
							TEXT("print('Frame range: %d-%d'); ")
								TEXT("res = le.export_selected_to_usd(r'%s', r'%s', start_frame=%d, end_frame=%d); ")
									TEXT("print('Export result:', res)"),
		*PyPath, *FilePath, *AssetLibraryPath, StartFrame, EndFrame,
		*FilePath, *AssetLibraryPath, StartFrame, EndFrame);
